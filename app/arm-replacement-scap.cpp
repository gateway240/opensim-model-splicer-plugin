...
int main() {
  ...
  const fs::path modelsDir = fs::path("./scapulothoracic");
  const fs::path outputDir = fs::path("../output/scapulothoracic");
  const std::string baseModelName = "gait2392_thelen2003muscle_Lerner_knee";
  const std::string armModelRightName = "ScapulothorachicJoint_Shoulder_v40600_r";
  const std::string armModelLeftName = "ScapulothorachicJoint_Shoulder_v40600_l";
  const std::string suffix = "scapulothoracic_arm";
  const std::string extOsim = "osim";

  const fs::path baseModelPath = (baseModelName + "." + extOsim);
  ...
  try {
    ...
    const std::string base_joint_name = "ground_thorax";
    ...
    const std::vector<std::string> joints = {"scapulothoracic_r", "sternoclavicular_r","scapulothoracic_l", "sternoclavicular_l"};
